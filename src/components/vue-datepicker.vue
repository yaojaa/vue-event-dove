...
<template>
    <div class="cov-vue-date w-full" :class="options.wrapperClass ? options.wrapperClass : {}">
        <div class="datepickbox ui icon input fluid">
            <input type="text" title="请输入日期时间" class="cov-datepicker" readonly="readonly" :placeholder="placeholder" v-bind:value="date" :required="required" @click="showCheck" @focus="showCheck" :style="options.inputStyle ? options.inputStyle : {}" :class="options.inputClass ? options.inputClass : {}" />
            <i class="calendar icon"></i>
        </div>
        <div class="datepicker-overlay" v-if="showInfo.check" @click="dismiss($event)" v-bind:style="{'background' : options.overlayOpacity? 'rgba(0,0,0,'+options.overlayOpacity+')' : 'rgba(0,0,0,0.5)'}">
            <div class="cov-date-body">
                ...
                <div class="cov-date-box" v-if="showInfo.day">
                    <div class="cov-picker-box">
                        <div class="week">
                            <ul>
                                <li v-for="weekie in library.week[options.language]">{{weekie}}</li>
                            </ul>
                        </div>
                        <div class="day" v-for="day,index in dayList" :key="index" @click="checkDay(day)" :class="{'checked':day.checked,'unavailable':day.unavailable,'passive-day': !(day.inMonth)}" :style="day.checked ? (options.color && options.color.checkedDay ? { background: options.color.checkedDay } : { background: '#09f' }) : {}">{{day.value}}</div>
                    </div>
                </div>
                ...
                <div class="cov-date-box list-box" v-if="showInfo.month">
                    <div class="cov-picker-box date-list">
                        <div class="date-item" v-for="monthItem,index in library.month[options.language]" :key="index" @click="setMonth(monthItem)">{{monthItem}}</div>
                    </div>
                </div>
                ...
                <div class="button-box">
                    <span @click="clear">{{options.buttons? options.buttons.clear : 'Clear' }}</span>
                    <span @click="showInfo.check=false">{{options.buttons? options.buttons.cancel : 'Cancel' }}</span>
                    <span @click="picked">{{options.buttons? options.buttons.ok : 'Ok'}}</span>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    data() {
        ...
        return {
            ...
            library: {
                week: {
                    'en': ['Mon', 'Tru', 'Thir', 'Fou', 'Fri', 'Sun', 'Sat'],
                    'zhCn': ['一', '二', '三', '四', '五', '六', '日']
                },
                month: {
                    'en': ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'],
                    'zhCn': ['一月', '二月', '三月', '四月', '五月', '六月', '七月', '八月', '九月', '十月', '十一月', '十二月']
                },
                year: []
            },
            checked: {
    ...
    methods: {
        ...
        showDay(time) {
            ...
            this.displayInfo.month = this.library.month[this.options.language][moment(this.checked.currentMoment).month()]
            let days = []
            ...
            for (let i = 1; i <= monthDays; ++i) {
                ...
                if (i === Math.ceil(moment(currentMoment).format('D')) && moment(oldtime, this.options.format).year() === moment(currentMoment).year() && moment(oldtime, this.options.format).month() === moment(currentMoment).month()) {
            ...
            for (let i = 0; i < firstDay - (this.options.SundayFirst ? 0 : 1); i++) {
                let passiveDay = {
        ...
        checkDay(obj) {
            ...
            if (this.options.type === 'day' || this.options.type === 'min') {
        ...
        setMonth(month) {
            let mo = (this.library.month[this.options.language].indexOf(month) + 1)
            if (mo < 10) {
                mo = '0' + '' + mo
            }
        ...
        picked() {
            ...
            if (this.options.type === 'day' || this.options.type === 'min') {
                ...
                date = moment(this.checked.currentMoment).format(this.options.format)
            } else {
