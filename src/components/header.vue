<template>
    <div id="header">
        <div class="ui top fixed  menu headBar">
            <div class="left menu">
                <router-link class="item noLine logo" to="/"><img src="/images/logo.png" alt="会鸽"></router-link>
                <router-link class="item noLine f16" to="/event">{{ $t("header.find") }}</router-link>
               <!--  <a class="item noLine f16" href="http://www.eventdove.com/live" target="_blank">会鸽互娱</a> -->
            </div>
            <div class="right menu">
                <!-- <div class="ui dropdown item noLine">
                    <div class="text">
                        <i v-if="currLang === 'zhCn'" class="united states flag"></i>
                        <i v-else class="china flag"></i>
                    </div>
                    <i class="dropdown icon"></i>
                    <div class="menu">
                        <a class="item" @click="setLanguage('zhCn')"><i class="china flag"></i>简体中文</a>
                        <a class="item" @click="setLanguage('en')"><i class="united states flag"></i>English</a>
                    </div>
                </div> -->
                <div class="item noLine creatBtn" id="creatBtn">
                    <router-link class="mini ui button blue" :to="{name:'create',params:{id:1,eventId:0}}">{{ $t("header.create") }}</router-link>
                </div>
                ...
                <div class="ui dropdown item nav-dropdown" v-show="userStatus.id">您好，
                    <!-- <a href="javascript:void(0)">{{userStatus.nickname || userStatus.username}}</a><i class="dropdown icon"></i> -->
...
export default {
    ...
    data() {
        return {
            currLang: window.localStorage.getItem('lang') || 'zhCn'
        }
    },
    computed: {
        ...mapGetters(['userStatus']),
        lang() {
            return this.currLang === 'zhCn' ? 'English?' : '简体中文?'
        }
    },
    methods: {
        logout() {
